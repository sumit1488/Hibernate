...
public class HibernateApplication {
	...
	public static void main(String[] args) {
		...
		Address userAddress1 = Address.builder().addressLine1("user_addrLine1").addressLine2("user_addressLine2").city("user_Pune")
				.state("user_Maharashtra").zipCode("user_411027").build();
		Address userAddress2 = Address.builder().addressLine1("user_addrLine2").addressLine2("user_addressLine2").city("user_Pune")
				.state("user_Maharashtra").zipCode("user_411027").build();
		...
		User user1 = User.builder().firstName("Anu").emailAddress("sumitjobs1488@gmail.com")
				.lastName(LastNameEnum.PAREEK).birthDate(LocalDate.of(1988, 9, 25)).lastUpdatedBy("Sumit")
				.lastUpdatedDate(null).createdBy("ETL").createdDate(LocalDate.now()).address(Arrays.asList(userAddress1,userAddress2)).build();
		...
		Credentials credentials = Credentials.builder().username("userName").password("pass").lastUpdatedBy("Sumit").lastUpdatedDate(null)
				.createdBy("ETL").createdDate(LocalDate.now()).user(user1).build();
		...
		Bank bank = Bank.builder().name("HDFC BANK").international(true).lastUpdatedBy("Sumit").lastUpdatedDate(null)
				.createdBy("ETL").createdDate(LocalDate.now()).address(address).contacts(Arrays.asList("mail", "phone", "email")).
				positionNameMap(positionMap).build();
		...
		Transaction transaction1 = Transaction.builder().closingBal(new BigDecimal(123123123.90)).initialBal(new BigDecimal(123123123.90)).build();
		Transaction transaction2 = Transaction.builder().closingBal(new BigDecimal(123123123.90)).initialBal(new BigDecimal(123123123.90)).build();
